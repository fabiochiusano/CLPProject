...
object Trees {
  ...
  case class ClassDecl(id: Identifier, parent: Option[Identifier], vars: List[VarDecl], methods: List[MethodDecl], nativeMethods: Option[List[NativeMethodDecl]]) extends Tree with Symbolic[ClassSymbol]
  ...
  case class NativeMethodDecl(retType: TypeTree, id: Identifier, args: List[Formal]) extends Tree with Symbolic[MethodSymbol] //Custom extension: new tree for native methods
